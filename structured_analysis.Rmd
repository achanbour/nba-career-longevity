...
data %>%
  ...
  ggplot(aes(x = Round_Drafted, y = proportion, fill = TARGET_5Yrs)) + geom_bar(stat = "identity", position = position_dodge()) + scale_x_discrete(limits = c(seq(1, 10), "U")) + ylab("for the outcome variable group, proportion of players drafted in the round")
...
data %>%
  ...
  group_by(TARGET_5Yrs) %>% mutate(proportion = n/sum(n)) %>%
  ggplot(aes(x = Position, y = proportion, fill = TARGET_5Yrs)) + geom_bar(stat = "identity", position = position_dodge()) + scale_x_discrete(limits = c("Point guard", "Shooting guard", "Center", "Power forward", "Small forward")) + ylab("proportion of players drafted in that round") + ylab("for the outcome variable group, proportion of players in the position")
```
...
# manipulate undrafted variable to make sure all other rounds get a dummy
...
The chosen predictors (which are continuous variables) capture the general performance of a player in both offence and defense without going into more sophisticated details.
...
data_logistic <- data_training %>% select(c(regressors, TARGET_5Yrs))
...
Using the results from the Lasso Regression, we want choose the following subset of regressors to train our KNN model: GP, MIN, PTS, FG., FTM, OREB, Win_Perc, Year_Debut, Round_Drafted_X1
