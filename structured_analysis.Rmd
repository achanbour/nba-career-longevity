...
Also, total rebounds = offensive rebounds + defensive rebounds, so we drop the total variable. 
...
We also drop the following variables:
  ...
  3) **Year drafted**, as this information is captured more effectively by the year debut variable:

```{r remove_collinear_and_uninformative}
data <- data_no_nas %>%
  select(Name, Year_Drafted, Team, -FGA, -X3PA, -FTA, -REB, -Year_Drafted)
```
...
# step_dummy doesn't give the first variable a dummy. We want this to be undrafted players, so we code it as 0 so it becomes the first value
...
dummy_multi_choice_rec <- recipe(~ ., data = data) %>%
  step_dummy(Round_Drafted) %>% step_dummy(Position) %>%
  prep()
...
data_to_split$debut_before_1989 <- ifelse(data_training_lasso$Year_Debut >= 1989, 0, 1)
...
```{r logistic_variable_selection}
# selecting the outcome and predictors from the train set
regressors <- c("GP", "PTS", "REB", "AST", "STL", "BLK", "TOV")

data_logistic <- data_training %>% select(c(all_of(regressors), TARGET_5Yrs))
...
y_gd <- data_training %>% select(TARGET_5Yrs)
...
```{r direct_logistic_regression}
logistic_fit <- glm(unlist(y_gd) ~ GP + PTS + REB + AST + STL + BLK + TOV - 1, data=data_logistic, family="binomial") #we omit the intercept
...
The model development section starts with a logistic regression model fit on all predictors, using tidymodels. As an introductory stage, the model will also be used to evaluate significance of our predictor variables according to the z-tests in the summary below.
...
kable(tidy(logistic_model)) %>% scroll_box(height = "400px") #%>% as_tibble() %>% print(n=40)
```
...
We can use the p-values shown in the output to conclude whether the variable coefficients are significant (at the 5% level). We see that the only significant coefficients (with p-value below 0.05) are:
...
As a next step, we check for non-linearity using another classification approach - that of Support Vector Machines (SVM). As opposed to logistic regression, SVM allows us to classify data points according to non-linear decision boundaries.
...
We now move onto a very different classification approach: decision-tree classification. Decision trees have the advantage of being very interpretable, allowing us to directly visualise the relationship between the outcome and the explanatory variables. Since the focus of this model is interpretability, we limit the tree depth to make the plot readable, and the flow of splits easier to understand. 
...
With 34 different regressors, this may not be the optimal set of regressors for our model. We want to complete some process of subset selection to not only draw some inference about which variables may be more significant, but also reduce the dimensionality of the KNN model as KNN is susceptible to the curse of dimensionality. 
...
-A good example of this is a player not in our dataset Markelle Fultz. Markelle was drafted first overall in the 2017 NBA draft but had a terrible season after developing a neurogenic syndrome that inhibited his shooting motion. Despite this, Markelle played the entire season and performed terribly. 
  
Our current specification does not accurately capture circumstances where confounding variables like injury/ health, team chemistry / personality fit or off-the-field status can impact the likelihood of a player reaching year 5. 
...
In other words, the effect for players drafted after 1989 is captured by $\beta_1$ and players drafted before 1989 is captured by $\beta_1 + \beta_2$
...
Using the results from the Lasso Regression, we want to choose the following subset of regressors to train our KNN model: GP, MIN, PTS, FG., FTM, OREB, Win_Perc, Year_Debut, Round_Drafted_X1
...
Looking at the Lasso results above, this subset of regressors seems to represent the variables that capture the most information regardless of the magnitude of the penalty and appear to be the most significant. 

```{r knn}
vars <- c("GP", "MIN", "PTS", "FG.", "FTM", "OREB", "Win_Perc", "Round_Drafted_X1", "Round_Drafted_1_before_1989")

data_training_KNN <- data_training_lasso %>%
  select(all_of(vars)) %>% bind_cols(outcome_training)

data_testing_KNN <- data_testing_lasso %>%
  select(all_of(vars)) %>% bind_cols(outcome_testing)
