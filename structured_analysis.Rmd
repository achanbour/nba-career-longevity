...
```{r tidymodels_knn}
vars <- c("GP", "MIN", "PTS", "FG.", "FTM", "OREB", "Win_Perc", "Year_Debut", "Round_Drafted_X1", "Round_Drafted_1_before_1989")

data_training_KNN <- data_training_lasso %>%
  select(vars) %>% bind_cols(outcome_training)

data_testing_KNN <- data_testing_lasso %>%
  select(vars) %>% bind_cols(outcome_testing)
