...
We also drop the player and team names at this point, as they serve no purpose in our analysis (the team should be captured by the team win percentage variable):
...
Train_Outcome <- data_training %>% select(TARGET_5Yrs)
Test_Outcome <- data_testing %>% select(TARGET_5Yrs)
```
...
```{r}
# Create Dummy variables for Year_debut_1989
Train_data_KNN$Year_debut_before_1989 <- ifelse(data_training$Year_Debut >= 1989, 0, 1)
Test_data_KNN$Year_debut_before_1989 <- ifelse(data_testing$Year_Debut >= 1989, 0, 1)
...
model_lasso <- glmnet(data.matrix(Train_data_KNN), y = data.matrix(Train_Outcome), intercept = FALSE, alpha = 1 , lambda = 10^seq(from = -6, to = 0, by = 0.5), family = "binomial")
...
cv_lasso <- cv.glmnet(x = data.matrix(Train_data_KNN), y = data.matrix(Train_Outcome), type.measure = "auc" , nfolds = 10)
...
```{r}
KNN_Predicted <- knn(train = data.matrix(Adj_Train_data_KNN), test = data.matrix(Adj_Test_data_KNN), cl = data.matrix(Train_Outcome), k = 25)
CrossTable(x= data.matrix(Test_Outcome), y= KNN_Predicted, prop.chisq = FALSE, chisq = TRUE)
```
