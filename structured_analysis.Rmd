...
tree <- decision_tree(tree_depth = 3) %>%
  set_engine("rpart") %>%
  set_mode("classification") %>%
  fit(TARGET_5Yrs ~ ., data = data_training)
...
With 40 different regressors, this may not be the optimal set of regressors for our model. We want to complete some process of subset selection to not only draw some inference about which variables may be more significant, but also reduce the dimensionality of the KNN model as KNN is susceptible to the curse of dimensionality. 
...
knn <- nearest_neighbor(mode = "classification", engine = "kknn", neighbors = round(sqrt(nrow(data_training)))) %>% 
