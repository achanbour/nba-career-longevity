...
This dataset contains the key performance metrics of NBA players in their rookie (first) season in the NBA League and additional information about how they entered the league.
...
Additional data regarding the year players were drafted, the round they were drafted in, the year they debuted, the team they played for and that team's record at the end of the season was web scraped from Wikipedia using a Python script we wrote.

## Variable descriptions

### Outcome
**TARGET_5Yrs**: Dummy variable with value 1 for players who went on to play five or more years in the league, and 0 for those that did not.
...
### Predictors
  ...
  20) **Year_Drafted** - Year the player declared for the draft [integer]
...
library(class)
library(rpart.plot)
library(kableExtra) # kable function to display data cleanly
library(prettydoc) # to use the "readthedown" Rmarkdown theme, renders nicely
raw_data <- read.csv("data_in/nba_logreg.csv") # read in raw data
raw_data$TARGET_5Yrs <- as.factor(raw_data$TARGET_5Yrs) # treat dummy outcome variable as binary factor
...
```{r omit_nas}
data_no_nas <- na.omit(raw_data)
sprintf("NAs omitted, number of obs falls from %s to %s", nrow(raw_data), nrow(data_no_nas))
```

## Remove collinear and irrelevant variables

For some aspects of game performance, such as free throws, we have 3 variables. One for attempts, one for successes, and one for percentage success $= 100*\frac{successes}{attempts}$. Including all three variables leads to collinearity when we come to build models, as each can be calculated directly from the other two. So, we drop the attempts variables to avoid multicollinearity issues.
...
The summary function does a good job of making sure the numeric values all fall within the expected ranges. However, non-numeric variables can also contain errors. We check for this by getting the distinct values for the two remaining columns with character type (Round Drafted and Position).

```{r check_character_variable_values}
data %>% count(Round_Drafted)
data %>% count(Position)
```
...
* Our dataset doesn't contain any players that were drafted in round 9. This is noted but not likely to cause issues because the sample is quite small, and round 9 didn't exist past 1989. This will be discussed again later on.

* Very few players are drafted in other high round numbers, such as round 10 (only 1 player). This may cause problems, because the factor level 10 can only exist in either the training or testing data, but not both.
...
As a simple way to get a sense of which of the numeric regressors are correlated with the outcome, we take the correlation coefficient of each numeric regressor with the outcome variable (TARGET_5Yrs).
...
* The correlation between the outcome and respecively win percentage and three pointer metrics appear to be close to 0.

* The correlation between the outcome and the year debut appears to be negative despite the fact that we would expect it to be 0.


## Distributions of character variables, by outcome

For the non-numeric variables, we can't use correlation coefficients. However, we can inspect the relationship between these variables and the outcome through bar charts.

```{r char_vars_bar_charts, warning=FALSE}
# plot Round Drafted counts by outcome status
data %>%
  ...
  ggplot(aes(x = Round_Drafted, y = proportion, fill = TARGET_5Yrs)) + geom_bar(
    stat = "identity", position = position_dodge()) + ylab(
      "proportion of the outcome group's players drafted in the round") + scale_x_discrete(
        limits = c(seq(1, 10), "U"))
# plot Position counts by outcome status
data %>%
  ...
  group_by(TARGET_5Yrs) %>%
  mutate(proportion = n/sum(n)) %>%
  ggplot(aes(x = Position, y = proportion, fill = TARGET_5Yrs)) + geom_bar(
    stat = "identity", position = position_dodge()) + ylab(
      "proportion of the outcome group's players in the position")
```

**Round drafted**

In basketball, we would expect better players to be drafted in an earlier round (with a lower round number) and the undrafted (U) players to be worse.

There is a clear difference in the distribution of round numbers for those who reached 5 years and those who did not. Among those with TARGET_5Yrs = 1, more than 65% were drafted in the first round, and fewer than 10% were undrafted.

**Position**

We see that there is a slight difference in distribution of positions between those who made 5 years and those who didn't.

We conclude that both round drafted and position are likely to be relevant.

# III: Pre-processing Dataset

We now move on to first preparing the data for, and then fitting, models. Where possible, we will do this using tidymodels methods to make it easier to fit and compare several models.

## Create indicator variables

Using the "step_dummy" function from recipes (part of tidymodels), we create dummy columns for the categorical variables (Round Drafted and Position). The function replaces each categorical variable by n-1 dummy columns, where n is the number of categories the variable can take, to avoid the dummy variable trap.

```{r create_indicators, warning=FALSE}
# step_dummy doesn't give the first value of the variable a dummy.
# For round drafted, it makes sense for this to be undrafted players
# so we code U as 0 to make it the first value
...
To correct for this change, we will need to create a time dummy variable and add interaction terms to the Round_Drafted_# dummy variables.
...
# create interaction columns for Round_Drafted_# with debut_before_1989
data_to_split[names] <- lapply(select(data_to_split,starts_with("Round_Drafted")),
                               function(x) x*data_to_split$debut_before_1989)
```
...
Using the "initial_split" function from rsample (part of tidymodels), we split the data into training and testing, using (the default) 75% of observations for training and 25% for testing. The same data and samples will be used by all models. To preserve the overall class distribution, we stratify on the outcome variable (TARGET_5Yrs).
...
data_testing <- data_split %>%
  testing()
sprintf("Data had %s obs, now split into training (%s obs) and testing (%s obs)",
        nrow(data), nrow(data_training), nrow(data_testing))
...
We begin by thinking about which metrics will be most useful to calculate for each model.

Our dataset is imbalanced, with 63% of players reaching 5 years, and 37% not reaching 5 years. Because of this, it would be misleading to use a simple accuracy metric. We will instead focus on **balanced** accuracy, which is the average of sensitivity (true positive rate) and specificity (true negative rate), which we will also inspect. This normalizes true positive and true negative predictions by the number of positive and negative observations respectively.

```{r define_evaluation_metrics, warning=FALSE}
evaluation_metrics <- metric_set(bal_accuracy, spec, sens)
```
...
kable(tidy(logistic_model) %>%
        arrange(p.value)) %>%
  scroll_box(height = "400px") #%>% as_tibble() %>% print(n=40)
```
...
We can use the p-values or t-test statistics shown in the output to conclude whether the variable coefficients are significant (ie. significantly different from 0). At the 5% significance level, the significant coefficients are those with p-values below 0.05 or with t-test statistic values above 1.9600. Based on the output of logistic regression, the significant variables are:
...
This is indeed a striking result: one might expect the game performance of a player to be indicative of his talent and therefore of his ability to go further in the NBA. Year debut being significant, there seems to be a time trend in the NBA drafting process whereby talented players tend to get drafted around the same time. A possible explanation is that drafting more talented players at the same time increases competitiveness in the league, pushing talented players to play even better and stay longer in the NBA. Additionally, college basketball may have improved over time meaning that prospects in recent years may be better than before.

However, we should note that all the variables related to points scored by a player in a game such as the total number of points, free throws, 3 pointers etc. are all highly correlated. In particular, the insignificance of free throw metrics may be a result of multicollinearity and not due the lack of association with the outcome.
Similarly, it may be argued that performance on offense and defense captured by variables such as steal, block, assist, rebound etc. exhibit correlation. Top players in the NBA are generally good in both offense and defense (for example Lebron James).

A final note on significance regarding categorical variables:
Logistic regression uses *reference level coding*. The t-test compares each of the non-reference categories to the reference (omitted) one. It is quite possible for none of those to be significant, but there to be significant differences amongst the non-reference categories.

**Predictions and model accuracy**

```{r logistic_evaluation, warning = FALSE}
# predictions
predictions_logistic <- logistic_model %>%
  predict(data_testing) %>%
  bind_cols(data_testing)
# evaluation
metrics_logistic <- evaluation_metrics(predictions_logistic, truth=TARGET_5Yrs, estimate = .pred_class) %>%
  mutate(model = "logistic")
kable(metrics_logistic)
```

The logistic regression model yields a balanced accuracy of 76.6%. For our classification task, this can be seen as a good score indicating that a linear classification boundary fits our data well.
...
For simplicity we omit the division by $\frac{1}{n}$ as it doesn't affect our minimisation problem.
...
logistic_loss <- function(X, y, beta) {
  ...
  y <- as.numeric(as.character(y)) # converting the factors of the outcome first to strings then to numeric
  loss <- 0
  for (i in 1:nrow(X)) {
    X_i_B <- sum(X[i, ] * beta)
    p_hat <- sigmoid(X_i_B)
...
gradient <- function(X, y, beta) {
  ...
  beta <- as.matrix(beta, length(beta), 1) # converting a beta to a column vector, length length(beta)
  as.matrix(t(X) %*%(sigmoid(X %*% beta) - y), ncol(X), 1) # return gradient as a column of length ncol(X)
}
...
The chosen predictors have p-value below 0.05 and capture the most prominent factors affecting the performance of a player in the NBA.
The predictors are: games played, winning percentage of the team, year of debut, drafted in the first round, field goals percentage, three pointers made and offensive rebound.


```{r apply_gradient_descent}
regressors_grad_desc <- c("GP", "Win_Perc", "Year_Debut", "Round_Drafted_X1", "FG.", "X3P.Made", "OREB")
data_grad_desc <- data_training %>%
  select(c(all_of(regressors_grad_desc),TARGET_5Yrs))
...
logistic_fit <- glm(unlist(y_gd) ~ GP + Win_Perc + Year_Debut + Round_Drafted_X1 + FG. + X3P.Made + OREB - 1,
                    data=data_grad_desc, family="binomial") # we omit the intercept
...
data_testing_lasso <- data_testing %>%
  select(-TARGET_5Yrs)
outcome_training <- data_training %>%
  select(TARGET_5Yrs)
outcome_testing <- data_testing %>%
  select(TARGET_5Yrs)
...
Most Notably:
  ...
  -Position_Point.guard (Point Guard position)
...
What perhaps is more interesting is the observation that defensive rebounds, steals and turnovers seem to have a relatively insignificant impact on a player's ability to reach year 5. Whilst there are players who pride themselves on their ability to defend (E.g. Draymond Green - 4x NBA All-Defensive First Team), in reality the structure of the game seems to lend itself more to offensive attributes and point scoring ability.
...
The data suggests a positive association between being drafted in the first round and the likelihood of making it to year 5. This is consistent with the notion that players drafted in the first round will have performed better in college or international leagues before declaring for the draft.
...
Using the results from the Lasso Regression, we want to choose the following subset of regressors to train our KNN model: GP, MIN, PTS ,FG., FTM, FT., OREB, AST, BLK, Win_Perc, Round_Drafted, Position_Point.guard, Round_Drafted_1_before_1989

Logistic regression suggests: GP, Win_Perc, Year_Debut, Round_Drafted_X1, FG., X3P.Made, OREB

LASSO suggests: GP, MIN, PTS, FG., FTM, FT., OREB, AST, BLK, Win_Perc, Round_Drafted_X1, Position_Point.guard, Round_Drafted_1_before_1989

What is notably absent from the lasso results compared to the logistic regression is the time effect of being drafted.
Whilst lasso suggests that shooting from the field, offensive rebounds, assists and blocks are the best way for players to make an impact. In general, scoring more points and spending time on the field is likely to improve the likelihood of retention.
...
```{r knn}
vars_lasso_output <- c("GP", "MIN", "PTS", "FG.", "FTM", "FT.", "OREB", "AST", "BLK", "Win_Perc",
                       "Round_Drafted_X1", "Position_Point.guard", "Round_Drafted_1_before_1989")
data_training_lasso_output <- data_training %>%
  select(c(all_of(vars_lasso_output),TARGET_5Yrs))
data_testing_lasso_output <- data_testing %>%
  select(c(all_of(vars_lasso_output),TARGET_5Yrs))
...
# evaluation
metrics_knn <- evaluation_metrics(predictions_knn, truth=TARGET_5Yrs, estimate = .pred_class) %>%
  mutate(model = "knn")
kable(metrics_knn)
```

KNN yields a balanced accuracy of 73.2%, which is reasonable considering it is a simple model. This suggests the LASSO variable selection worked well.
...
svm_rbf <- svm_rbf() %>%
  ...
  fit(TARGET_5Yrs ~ ., data = data_training)
# predictions
predictions_svm_rbf <- svm_rbf %>%
  predict(data_testing) %>%
  bind_cols(data_testing)
# evaluation
metrics_svm_rbf <- evaluation_metrics(predictions_svm_rbf, truth=TARGET_5Yrs, estimate = .pred_class) %>%
  mutate(model = "svm_rbf")
kable(metrics_svm_rbf)
```

The RBF Kernel yields a balanced accuracy of 73.7%.
...
predictions_svm_poly <- svm_poly %>%
  ...
  bind_cols(data_testing)
# evaluation
metrics_svm_poly <- evaluation_metrics(predictions_svm_poly, truth=TARGET_5Yrs, estimate = .pred_class) %>%
  mutate(model = "svm_poly")
kable(metrics_svm_poly)
```

The polynomial kernel yields a balanced accuracy of 75.7% which is an improvement on the radial kernel SVM model.
...
predictions_tree <- tree %>%
  ...
  bind_cols(data_testing_lasso_output)
# evaluation
metrics_tree <- evaluation_metrics(predictions_tree, truth=TARGET_5Yrs, estimate = .pred_class) %>%
  mutate(model = "decision_tree")
kable(metrics_tree)
```

### Decision tree - interpretation of results

* Games played appears to be the most important factor determining the success of an NBA player. Players who play more games in a season tend to be better integrated and may play a more impactful role in the strategy of the team. Playing more games in a season will allow players to get more experience and gain confidence not just in themselves but also from the coaches and fans.
* Round_Drafted_X1 is a second important factor. This is consistence with the results above. We could argue that first round draft picks have a high ceiling and are generally more talented.
* Free throws made and percentage are both important as they reflect the player's ability to handle stressful situations and their understanding of the rules and how to draw fouls.
* Win percentage is another important factor as stronger teams are less likely to trade players away as doing so could risk hurting chemistry or schematic advantages.
...
* The tree classifier yields a balanced accuracy of 72.7%.
* When compared with the baseline logistic regression model, the decision tree appears to marginally underperform. However, when considering the interpretations, the evidence suggests that these models are consistent. Looking at the baseline logistic regression coefficients, we see that the coefficients for GP, Win_Perc and Round_Drafted_X1 are statistically significant at the 5% significance level and are positive implying that increases in these variables are associated with an increase likelihood of making year 5.
* The relatively lower predictive accuracy is compensated by the simplicity and the interpretability of the decision tree model. Since there are several other factors affecting a player's success in the NBA, some of which we can't control for in our models (such as genetic attributes, injury, nationality etc.), it is not surprising that our decision tree does not achieve an exceptionally high predictive accuracy. For what it is, the model seems to be performing reasonably well.
...
```{r random_forest}
set.seed(1423) # set seed to fix outcome of random forest, as it uses random sampling
...
predictions_rf <- rf %>%
  ...
  bind_cols(data_testing)
# evaluation
metrics_rf <- evaluation_metrics(predictions_rf, truth=TARGET_5Yrs, estimate = .pred_class) %>%
  mutate(model = "random_forest")
kable(metrics_rf)
```

Random forest achieves a balanced accuracy of 77.5%, which is higher than our previous single decision tree classifier, as expected.

# V: Conclusion

```{r comparison_plot}
list = mget(ls(pattern="^metrics_")) # get all relevant dataframes (metrics_XXX)
all_metrics = bind_rows(list) %>%
  pivot_wider(names_from = .metric, values_from = .estimate) #%>%
  # rename(model = model, "Balanced Accuracy" = bal_accuracy, "Specifity" = spec, "Sensitivity" = sens)
ggplot(all_metrics, aes(x = spec, y = sens, col = bal_accuracy)) + geom_point(size = 5) + geom_text(
  aes(label = model), size = 4, hjust = 1.15, vjust = 1.15) + scale_color_gradient(
    low = "red", high = "green") + xlim(0.76, NA) + ylim(0.58, NA) + labs(
      x ="Specificity", y ="Sensitivity", col = "Balanced Accuracy")
```

We started this project with the hopes of better understand what factors contribute to the success of NBA Rookies and developing a model to predict the success of players. Our work above suggests that the key factors that contribute to an NBA Rookie’s success appear to revolve around a player’s ability to influence a game and the expectations around them.

The results above regarding games played and other scoring metrics, reflect the ability of a player to influence games and the trajectory of a team through the season. We could argue that players who play more games are more likely to learn more and gain experiences that prove to be valuable as they develop.

The information captured above regarding the impact of Win Percentage and Round Drafted, reflect the influence of expectations on players and what effect this has on their ability to thrive in the league. We could argue that teams that are winning are generally less likely to question their personnel decisions compared to losing teams. Also, we would argue that 1st round draft picks have much more leeway as coaches and general managers may believe that they have a higher ceiling.
Based on the models we have built above, we could conclude that our random forest model would perform best in predicting the success of players based on their first year performance.

Whilst we have considered several variables in our project. Again, there will be several factors that impact the likelihood that a player succeeds that we have not considered. Therefore, we cannot draw any causal conclusions about the relationship established above.

Additionally, there are other nuances and perspectives that could be considered to further improve the accuracy of our models. For example, we did not consider the possibility that some teams may simply have a preference or general approach for drafting and developing talent. It is possible that these teams are more inclined to retain players regardless of their performance under certain conditions. Whilst this hypothesis is less likely to persist through time as General managers will have different philosophies, it is not impossible that some teams establish a culture of sorts, especially if ownership remains consistent.

To develop this project further, getting granular data on player performance would allow us to measure characteristics like consistency or and absences due to injury or other reasons. Furthermore, collecting more data on what pick a player was in the draft could prove to be very insightful. Although we would have to account for the fact the NBA Draft changed to a lottery system in 2017 after teams were intentionally losing games to improve their position in the draft.
