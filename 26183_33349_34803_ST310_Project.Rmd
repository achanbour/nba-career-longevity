...
data %>%
  ...
  ggplot(aes(x = Round_Drafted, y = proportion, fill = TARGET_5Yrs)) + geom_bar(stat = "identity", position = position_dodge()) + ylab("proportion of players drafted in the round, by outcome") # + scale_x_discrete(limits = c(seq(1, 10), "U"))
...
KNN yields an accuracy of around 76%, which is reasonable considering it is a simple model. This suggests the LASSO variable selection worked well.
...
The RBF Kernel yields a predictive accuracy of around 77%.
...
The polynomial kernel yields an accuracy of nearly 79% which is a very marginal improvement on the radial kernel SVM model.
...
Random forests achieve a predictive accuracy of nearly 80% which is higher than our previous single decision tree classifier, as expected.
