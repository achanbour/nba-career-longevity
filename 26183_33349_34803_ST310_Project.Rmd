...
While the correlation coefficient only measures linear relationships, we conclude that metrics involving three-pointers are likely to be poor predictors of success. Interestingly, so too is team win percentage - suggesting players cannot simply ride the coattails of their team to achieve individual success.
...
The chosen predictors have p-value below 0.05 aned capture the most prominant factors affecting the performance of a player in the NBA.
...
KNN yields an accuracy of around 73.2%, which is reasonable considering it is a simple model. This suggests the LASSO variable selection worked well.
...
The polynomial kernel yields an accuracy of 75.7%.
...
* The tree classifier yields an overall predictive accuracy of 72.7%.
