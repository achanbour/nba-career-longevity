...
For some aspects of game performance, such as free throws, we have 3 variables. One for attempts, one for successes, and one for percentage success = 100*(attempts/successes). Including all three variables leads to collinearity when we come to build models, as each can be calculated directly from the other two. So, we drop the attempts variables to avoid multicollinearity issues.
...
**Round drafted**
  ...
  2) Our dataset doesn't contain any players that were drafted in round 9. This is noted but not likely to cause issues because the sample is quite small, and round 9 didn't exist past 1989. This will be discussed again later on.
...
data %>%
  ...
  ggplot(aes(x = Round_Drafted, y = proportion, fill = TARGET_5Yrs)) + geom_bar(
    stat = "identity", position = position_dodge()) + ylab(
      "proportion of the outcome group's players drafted in the round") # + scale_x_discrete(limits = c(seq(1, 10), "U"))
...
data %>%
  ...
  ggplot(aes(x = Position, y = proportion, fill = TARGET_5Yrs)) + geom_bar(
    stat = "identity", position = position_dodge()) + ylab(
      "proportion of the outcome group's players in the position") #+ scale_x_discrete(limits = c("Point guard", "Shooting guard", "Center", "Power forward", "Small forward"))
```
...
data_to_split[names] <- lapply(
  select(data_to_split,starts_with("Round_Drafted")), function(x) x*data_to_split$debut_before_1989)
```
...
Whilst lasso suggests that shooting from the field, offensive rebounds, assists and blocks are the best way for players to make an impact. In general, scoring more points and spending time on field is likely to improve the likelihood of retention.
...
```{r knn}
vars_lasso_output <- c("GP", "MIN", "PTS", "FG.", "FTM", "FT.", "OREB", "AST", "BLK", "Win_Perc", "Round_Drafted_X1", "Position_Point.guard", "Round_Drafted_1_before_1989")
...
```
The polynomial kernel yields a balanced accuracy of 75.7% which is a marginal improvement on the radial kernel SVM model.
...
Random forests achieve a balanced accuracy of 77.5% which is higher than our previous single decision tree classifier, as expected.
...
list = mget(ls(pattern="^metrics_"))
all_metrics = bind_rows(list) %>%
  pivot_wider(names_from = .metric, values_from = .estimate) #%>%
  # rename(model = model, "Balanced Accuracy" = bal_accuracy, "Specifity" = spec, "Sensitivity" = sens)

# plot <- plot_ly(all_metrics, x = ~spec, y = ~sens, text = ~model, type = 'scatter', mode = 'markers',
#         marker = list(size = ~bal_accuracy, opacity = 0.5))
#
# plot <- plot %>% layout(title = 'Gender Gap in Earnings per University',
#          xaxis = list(showgrid = FALSE),
#          yaxis = list(showgrid = FALSE))
#
# plot

# ggplot(data = all_metric, aes(x = vis.df[,4],y = vis.df[,3], col = vis.df[,2])) +
#   geom_point(size = 3) +
#   geom_text(aes(label = vis.df[,1]),hjust = -0.2 , vjust = 0.4,size =2.5) +
#   labs(title = "Comparison of Model Performance", x ="Sensitivity", y ="Specificity", col = "Balanced Accuracy", size = "Balanced Accuracy")+
#   xlim(0.65,1.002)+
#   ylim(0.983,1.002)+
#   scale_color_gradient(low = "yellow", high = "red")

  # ggplot(aes(x = Round_Drafted, y = proportion, fill = TARGET_5Yrs)) + geom_bar(stat = "identity", position = position_dodge()) + ylab("proportion of players drafted in the round, by outcome")
...
ggplot(all_metrics, aes(x = spec, y = sens, col = bal_accuracy)) + geom_point(size = 5) + geom_text(
  aes(label = model), size = 4, hjust = 1.15, vjust = 1.15) + scale_color_gradient(
    low = "red", high = "green") + xlim(0.76, NA) + ylim(0.58, NA)
...
Based on the models we have built above, we could conclude that our SVM model would perform best in predicting the success of players based on their first year performance.
