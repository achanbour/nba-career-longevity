...
```{r}
#Reading: ISLR page 304-5 Hitter data example
#x: predictor
#y:outcome
#Given split on predictor space, compute RSS =sum RSS to left and right of split
tree_split <- function(x, y, min.obs = 10) {
  ...
  for (i in min.obs:(n-min.obs)) {
    Y_left <- Y[1:i]
    Y_right <- Y[(i+1):n]
     #***The predictions for Y for observations 1:i is given by the sample mean of Y[1:i], and similarly predictions for observations i+1:n is given by the sample mean of y[(i+1):n]. Compute the RSS for both sets of observations 1:i and i+1:n and store it as the ith element of RSSs
    RSSs[i] <- sum((Y_left - mean(Y_left))^2) +
      sum((Y_right - mean(Y_right))^2)
  }
  X[which.min(RSSs)] #split that min RSS
}
```
...
```{r}
inds1 <- which(x <= split1)   #cases less or = than split1
inds2 <- setdiff(1:length(x), inds1) #cases greater than split1
```
