...
rbfdot: radial basis kernel
C=cost parameter
cost argument quantifies the penalty associated with having an observation on the wrong side of the classification boundary.

Simulation example on how changes in parameters in SVM affects fit
http://uc-r.github.io/svm
https://rstudio-pubs-static.s3.amazonaws.com/220432_85cf9525136b43c8a19f5552b5fe5417.html
...
```{r}
#Colour gradient showing how confidently a new point would be classified and based features.
#Support vectors are marked by filled in points
#The two shapes represent the different classes
plot(k_fit)
```
...
### Role of sigma
```{r}
#Higher values of sigma makes fit more localised so can overfit (and see comments in the summary)
plot(train1d$x, train1d$y)

for(i in c(2,10)) {
        ksvm_fit <- ksvm(y~x,kernel = "rbfdot", data = train1d,sigma = i)
        pred <- predict(ksvm_fit,train1d_grid)
        points(x = train1d_grid$x, y = pred, type = 'l',
              col = i)
}
```
